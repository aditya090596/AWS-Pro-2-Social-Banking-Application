...
public interface ICustomerRepository extends JpaRepository<Customer, String>{
	...
	@Query(value="select * from customers u where u.email like %:emailParam%", nativeQuery=true)
	Optional<Customer> findByEmail(@Param("emailParam") String email);

	@Query(value="select * from customers u where u.email like %:email% and u.password like %:password%", nativeQuery=true)
